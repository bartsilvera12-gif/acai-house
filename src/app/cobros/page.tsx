...
type Resumen = { total_pendiente: number; total_vencido: number; cobrado_mes: number };
...
const FILTROS = [
  { id: "", label: "Todas" },
  { id: "f:vencidas", label: "Vencidas" },
  { id: "f:por_vencer", label: "Por vencer" },
  { id: "e:pendiente", label: "Pendientes" },
  { id: "e:parcial", label: "Parciales" },
  { id: "e:pagado", label: "Pagadas" },
];
...
export default function CobrosPage() {
  ...
  const [resumen, setResumen] = useState<Resumen>({ total_pendiente: 0, total_vencido: 0, cobrado_mes: 0 });
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      const qs =
        filtro.startsWith("f:") ? `?filtro=${filtro.slice(2)}` : filtro.startsWith("e:") ? `?estado=${filtro.slice(2)}` : "";
      const res = await fetchWithSupabaseSession(`/api/cobros/cuentas${qs}`, { cache: "no-store" });
      ...
      setResumen((body.data?.resumen ?? { total_pendiente: 0, total_vencido: 0, cobrado_mes: 0 }) as Resumen);
    } catch {
  ...
  async function registrarCobro() {
    ...
    try {
      const res = await fetchWithSupabaseSession("/api/cobros", {
        ...
        body: JSON.stringify({
          cuenta_por_cobrar_id: cobrando.id,
          monto: m,
          metodo_pago: metodo,
      ...
      setErrorCobro("Error de red al registrar el cobro.");
    } finally {
      setGuardandoCobro(false);
    }
  }

  const cuentasVisibles = useMemo(() => cuentas, [cuentas]);

  return (
    <div className="space-y-6">
      {toast && (
        <div className="fixed top-4 right-4 z-50 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium text-white shadow-lg">
          ✓ {toast}
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Cobros</h1>
          <p className="text-gray-600">Cuentas por cobrar y registro de pagos de clientes.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-1 text-2xl font-bold text-slate-800">{fmtGs(resumen.total_pendiente)}</div>
        </div>
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 shadow-sm">
          <div className="text-xs uppercase tracking-wide text-red-500">Vencido</div>
          <div className="mt-1 text-2xl font-bold text-red-700">{fmtGs(resumen.total_vencido)}</div>
        </div>
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
          <div className="text-xs uppercase tracking-wide text-emerald-600">Cobrado este mes</div>
          <div className="mt-1 text-2xl font-bold text-emerald-700">{fmtGs(resumen.cobrado_mes)}</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-gray-500">No hay cuentas por cobrar para este filtro.</div>
        ) : (
                ...
                {cuentasVisibles.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 px-4 text-gray-700">
                      <Link href={`/clientes/${c.cliente_id}/estado-cuenta`} className="hover:text-[#4FAEB2] hover:underline">
                        {c.cliente_nombre}
                      </Link>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${ESTADO_BADGE[c.vencida ? "vencido" : c.estado] ?? ESTADO_BADGE.pendiente}`}>
          ...
          <div className="w-full sm:max-w-md rounded-t-2xl sm:rounded-xl bg-white shadow-xl">
            <div className="border-b border-slate-200 px-5 py-4">
              ...
              <p className="text-xs text-gray-500">
                {cobrando.numero_venta} · {cobrando.cliente_nombre} · Saldo {fmtGs(cobrando.saldo, cobrando.moneda)}
              </p>
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 border-t border-slate-200 px-5 py-4">
              ...
              <button onClick={registrarCobro} disabled={guardandoCobro} className="inline-flex items-center justify-center gap-1.5 rounded-md bg-[#4FAEB2] px-5 py-2 text-sm font-medium text-white hover:bg-[#3F8E91] disabled:opacity-50">
                {guardandoCobro ? <><Loader2 className="h-4 w-4 animate-spin" /> Guardando…</> : "Confirmar cobro"}
              </button>
            </div>
          </div>
        </div>
