-- Fase 2 Gerencia: clasificación + views read-only en acaihouse. NO toca datos transaccionales.
BEGIN;

-- 1) Tabla de clasificación de planes/servicios
CREATE TABLE IF NOT EXISTS acaihouse.plan_categoria (
...
GRANT SELECT, INSERT, UPDATE, DELETE ON acaihouse.plan_categoria TO authenticated, service_role;
GRANT SELECT ON acaihouse.plan_categoria TO anon;

-- seed idempotente: limpiar y reclasificar todos los planes existentes
DELETE FROM acaihouse.plan_categoria;
INSERT INTO acaihouse.plan_categoria (plan_id, categoria, naturaleza, label)
SELECT p.id,
  CASE
...
FROM acaihouse.facturas f
LEFT JOIN acaihouse.suscripciones s ON s.id = f.suscripcion_id
LEFT JOIN acaihouse.plan_categoria pc ON pc.plan_id = s.plan_id AND pc.activo
LEFT JOIN LATERAL (SELECT string_agg(lower(descripcion), ' ') d FROM acaihouse.factura_items fi WHERE fi.factura_id = f.id) it ON true;

-- 3) v_revenue_mensual
CREATE OR REPLACE VIEW acaihouse.v_revenue_mensual AS
WITH fact AS (
...
CREATE OR REPLACE VIEW acaihouse.v_revenue_por_categoria AS
...
FROM acaihouse.facturas f JOIN acaihouse.v_factura_categoria fc ON fc.factura_id = f.id
WHERE f.estado <> 'Anulado'
GROUP BY 1,2,3;

-- 5) v_mrr
CREATE OR REPLACE VIEW acaihouse.v_mrr AS
...
FROM acaihouse.suscripciones s
LEFT JOIN acaihouse.plan_categoria pc ON pc.plan_id = s.plan_id AND pc.activo
GROUP BY 1,2;

-- 6) v_clientes_recurrentes
CREATE OR REPLACE VIEW acaihouse.v_clientes_recurrentes AS
...
FROM acaihouse.facturas f
JOIN acaihouse.clientes cl ON cl.id = f.cliente_id
LEFT JOIN acaihouse.v_factura_categoria fc ON fc.factura_id = f.id
WHERE f.estado <> 'Anulado'
GROUP BY 1,2,3
HAVING count(DISTINCT date_trunc('month', f.fecha)) >= 2;

-- 7) v_cuentas_por_cobrar
CREATE OR REPLACE VIEW acaihouse.v_cuentas_por_cobrar AS
...
FROM acaihouse.facturas f JOIN acaihouse.clientes cl ON cl.id = f.cliente_id
WHERE f.estado = 'Pendiente';

-- grants de las views
GRANT SELECT ON acaihouse.v_factura_categoria, acaihouse.v_revenue_mensual, acaihouse.v_revenue_por_categoria,
  acaihouse.v_mrr, acaihouse.v_clientes_recurrentes, acaihouse.v_cuentas_por_cobrar
